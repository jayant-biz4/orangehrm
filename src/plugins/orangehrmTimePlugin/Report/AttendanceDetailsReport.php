...
class AttendanceDetailsReport implements EndpointAwareReport
{
    use UserRoleManagerTrait;
    use TextHelperTrait;
    use I18NHelperTrait;

    public const PARAMETER_EMPLOYEE_NO = 'empNumber';
    public const PARAMETER_EMPLOYEE_NAME = 'employeeName';
    public const PARAMETER_PUNCHIN_DATE = 'punchInDate';
    public const PARAMETER_PUNCHIN_TIME = 'punchInTime';
    public const PARAMETER_PUNCHOUT_TIME = 'punchOutTime';
    public const PARAMETER_WORKHOURS = 'workHours';
    ...
    public const DEFAULT_COLUMN_SIZE = 5;
    ...
    {
        $filterParams = new AttendanceDetailsReportSearchFilterParams();
    ...
    public function getValidationRule(EndpointProxy $endpoint): ParamRuleCollection
    {
        return new ParamRuleCollection(
            ...
            ...$endpoint->getSortingAndPaginationParamsRules(
                AttendanceDetailsReportSearchFilterParams::ALLOWED_SORT_FIELDS
            )
        );
    }
    ...
    public function getHeaderDefinition(): HeaderDefinition
    {
        return new Header(
            [
                (new Column(self::PARAMETER_EMPLOYEE_NO))->setName($this->getI18NHelper()->transBySource('Employee Id'))
                ...
                (new Column(self::PARAMETER_EMPLOYEE_NAME))->setName($this->getI18NHelper()->transBySource('Employee Name'))
                    ->setSize(self::DEFAULT_COLUMN_SIZE),
                (new Column(self::PARAMETER_PUNCHIN_DATE))->setName($this->getI18NHelper()->transBySource('PunchIn Date'))
                    ->setCellProperties(['class' => ['col-alt' => true]])
                    ->setSize(self::DEFAULT_COLUMN_SIZE),
                (new Column(self::PARAMETER_PUNCHIN_TIME))->setName($this->getI18NHelper()->transBySource('In Time'))
                    ->setCellProperties(['class' => ['col-alt' => true]])
                    ->setSize(self::DEFAULT_COLUMN_SIZE),
                (new Column(self::PARAMETER_PUNCHOUT_TIME))->setName($this->getI18NHelper()->transBySource('Out Time'))
                    ->setCellProperties(['class' => ['col-alt' => true]])
                    ->setSize(self::DEFAULT_COLUMN_SIZE),
                (new Column(self::PARAMETER_WORKHOURS))->setName($this->getI18NHelper()->transBySource('Work Hours (Hours)'))
                    ->setCellProperties(['class' => ['col-alt' => true]])
                    ->setSize(self::DEFAULT_COLUMN_SIZE),
            ]
        );
    ...
    /**
     * @param AttendanceDetailsReportSearchFilterParams $filterParams
     * @return AttendanceDetailsReportData
     */
    public function getData(FilterParams $filterParams): ReportData
    {
        return new AttendanceDetailsReportData($filterParams);
    }
